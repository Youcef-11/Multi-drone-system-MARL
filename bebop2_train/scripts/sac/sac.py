...
# Implementation taken from openai spinnup https://github.com/openai/spinningup and adapted for our model
...
def sac(env_fn, actor_critic=core.mlp_actor_critic, ac_kwargs=None, seed=0,
        ...
        save_freq=int(1e4), save_path=None,load_path=None,  episode = 0):
...
def save(folder = "Models_sac", *models):
    ...
    for m in models:
        try:
            m[1].save_weights(f"{path}/{m[0]}.h5")
        except:
            print(f"{m[0]} not saved.")
