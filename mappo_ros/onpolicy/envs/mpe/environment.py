...
class MultiAgentEnv(gym.Env):
    ...
    def _set_action(self, action, agent, action_space, time=None):
        ...
        agent.action.c = np.zeros(self.world.dim_c)
        print(f"agent.action.c = {agent.action.c}")
        # process action
        if isinstance(action_space, MultiDiscrete):
            ...
            for s in size:
                ...
                index += s
            action = act
            print(f"action multi_discrete = {action}")
        else:
            action = [action]
        ...
        if agent.movable:
            ...
            if self.discrete_action_input:
                agent.action.u = np.zeros(self.world.dim_p)
                print(f"before, agent.action.u = {agent.action.u}")
                ...
                if action[0] == 4:
                    agent.action.u[1] = +1.0
                d = self.world.dim_p
                print(f"after, agent.action.u = {agent.action.u}")
            else:
                if self.discrete_action_space:
                    agent.action.u[0] += action[0][1] - action[0][2]
                    agent.action.u[1] += action[0][3] - action[0][4]
                    d = 5
                    print(f"discrete_action_space = {agent.action.u}")
                else:
                    ...
                    d = self.world.dim_p
                    print(f"force_discrete_action = {agent.action.u}")

            sensitivity = 5.0
            if agent.accel is not None:
                sensitivity = agent.accel
            agent.action.u *= sensitivity
            print(f"sensitivity = {agent.action.u}")
        ...
        print(f"end, agent.action = {agent.action.u}")
        print(f"end = {action}")
