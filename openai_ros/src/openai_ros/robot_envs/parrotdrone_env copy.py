...
class Bebop2Env(robot_gazebo_env.RobotGazeboEnv):
    ...
    def __init__(self, ros_ws_abspath):
        ...
        * /drone/land: Publish to make ParrotDrone Land
        ...
        ROSLauncher(rospackage_name="rotors_gazebo",
                    launch_file_name="mav_1_bebop.launch",
        ...
        # We will have to change these topic for the real bebop
        # See topic SpeedChanged for the velocity, Odom for the position

        rospy.Subscriber("/bebop2/camera_base/image_raw", Image, self.image_raw_callback)
        rospy.Subscriber("/bebop2/ground_truth/odometry", Odometry, self._odom_callback)
        rospy.Subscriber("/bebop2/fake_driver/pose", Pose, self._pose_callback)
        # rospy.Subscriber("/drone/gt_vel", Twist, self._gt_vel_callback)

        self._cmd_vel_pub = rospy.Publisher('/cmd_vel', Twist, queue_size=1)
        self._takeoff_pub = rospy.Publisher( '/bebop2/takeoff', Empty, queue_size=1)
        self._land_pub = rospy.Publisher('/bebop2/land', Empty, queue_size=1)
    ...
    def check_image_raw_ready(self):
        self.image_raw = None
        rospy.logdebug(
            "Waiting for /bebop2/camera_base/image_raw to be READY...")
        while self.image_raw is None and not rospy.is_shutdown():
            try:
                self.image_raw = rospy.wait_for_message(
                    "/bebop2/camera_base/image_raw", Image, timeout=5.0)
                rospy.logdebug("Current /bebop2/camera_base/image_raw READY=>")

            except:
                rospy.logerr(
                    "Current image_raw not ready yet, retrying for getting image_raw")
        return self.image_raw


    def check_odom(self):
        self.odom = None
        rospy.logdebug("Waiting for /bebop2/odom to be READY...")
        while self.odom is None and not rospy.is_shutdown():
            try:
                self.odom = rospy.wait_for_message(
                    "/bebop2/ground_truth/odometry", Pose, timeout=5.0)
                rospy.logdebug("Current odom READY=>")

            except:
                rospy.logerr(
                    "Current odom not ready yet, retrying for getting gt_pose")

        return self.odom

    def check_pose(self):
        self.pose = None
        rospy.logdebug("Waiting for /bebop2/pose to be READY...")
        while self.odom is None and not rospy.is_shutdown():
            try:
                self.pose = rospy.wait_for_message(
                    "/bebop2/fake_driver/pose", Pose, timeout=5.0)
                rospy.logdebug("Current pose READY=>")
            except:
                rospy.logerr(
                    "Current pose not ready yet, retrying for getting gt_pose")
        return self.pose
